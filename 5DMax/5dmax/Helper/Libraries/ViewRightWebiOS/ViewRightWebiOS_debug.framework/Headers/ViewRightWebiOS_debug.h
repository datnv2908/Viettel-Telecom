...
//! Project version number for ViewRightWebiOS_debug.
FOUNDATION_EXPORT double ViewRightWebiOSVersionNumber;

//! Project version string for ViewRightWebiOS_debug.
FOUNDATION_EXPORT const unsigned char ViewRightWebiOSVersionString[];
...
class ViewRightWebiOS
{
    public:
        ...
        //HLS Decrypt
        VRWebiOSError_t Decrypt(unsigned char * data, unsigned long & dataSize, MethodInfo decryptMethod, const char * keyfileURL, const unsigned char * keyIV, unsigned int keyIVSize = 16 , StreamingFormat format = StreamingFormat::HLS_FMP4);
        // Add this to the AVPlayer's AVAssetResourceLoaderDelegate's shouldWaitForLoadingOfRequestedResource method
        VRWebiOSError_t FPSKeyResourceLoader(AVAssetResourceLoadingRequest * loadingRequest, BOOL isFPSOfflineEnabled);
         ViewRightWebiOS::VRWebiOSError_t ReadFromPersistentStorage(NSString * keySaveLocation,AVAssetResourceLoadingRequest * loadingRequest,NSData* persistentContentKeyDataOffline,string resourceID,unsigned long expirationTime);
        ...
        void SetOutputControlSettings(struct OutputControls settings); // New api added for the debug version of library to set the output controls directly
        bool m_CustomerOutputControlSettingSet;
        OutputControls m_iOSSettingsCostumer;

        void SetKey(unsigned char * key);
        unsigned char m_ckey[16];
        bool m_bkeySet;
...
//      unsigned long                   m_fileCreationTime;
};
