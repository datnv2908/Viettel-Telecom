...
import { HTMLElementModel, HTMLContentModel, RenderHTML, defaultHTMLElementModels, MixedStyleRecord, useContentWidth, CustomTextualRenderer } from 'react-native-render-html';
import { useThemeManager } from '../../hooks';


export function ArticleScreenMobileBase({ slug = '' }: { slug?: string }) {
    ...
    <Box position="relative" flex={1}>
      ...
      <ScrollView>
        ...
        <Box bg="defaultBackground" position="relative">
          <Box padding={3}>
            <Text fontWeight="bold" fontSize={3} color={theme === 'dark' ? 'white' : 'black'} mb={1} width={'100%'}>
              {article?.title}
            </Text>
            ...
            <Box backgroundColor={theme === 'dark' ? 'white' : 'white'}>
              <RenderHTML
                baseStyle={{
                  paddingHorizontal: 8,
                }}
...
export default function ArticleScreen() {
  ...
  return <ArticleScreenMobileBase slug={route.params?.slug ?? ''} />;
}
