...
export function TrimmerBaseForWap(
    ...
    const [totalDurationAudio, setTotalDurationAudio] = useState<number>();
    const getData = async () => {
        try {
            const jsonValue = await AsyncStorage.getItem('@storage_Key')
            if (jsonValue) {
                setValueObj(JSON.parse(jsonValue))
                setTotalDurationAudio(JSON.parse(jsonValue).duration)
            }
        } catch (e) {
            console.log(e);
        }
    ...
    useEffect(() => {
        Promise.all([dataUrlToFile(valueObj?.uri, valueObj?.name)]).then(data => {
    ...
    const doPlay = async () => {
        ...
        try {
            await soundObj.current.loadAsync({ uri: valueObj.uri });
                                ...
                                <Box width={"100%"}>
                                    ...
                                    >
                                        <TouchableOpacity
                                            ...
                                            onPress={actionRbt}
                                            disabled={createLoadingRbtUnavailable}>
