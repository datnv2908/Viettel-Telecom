//
//  FreeCategoryViewController.swift
//  MeuClip
//
...
class FreeCategoryViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate, VideoTableViewCellDelegate, VideoSmallTableViewCellDelegate, UIScrollViewDelegate{
   ...
   @objc func refreshData() {
       ...
       getData(pager: pager) { (_ result: Result<APIResponse<[ContentModel]>>) in
   ...
   func getData(pager: Pager, _ completion: @escaping (Result<APIResponse<[ContentModel]>>) -> Void) {
      let category = "video_free"
   ...
   func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       ...
       if let cell = cell as? VideoSmallImageTableViewCell {
          if data[indexPath.row].price_play == "0" {
             cell.freeVideoImg.isHidden = false
          }
           cell.delegate = self
       }
