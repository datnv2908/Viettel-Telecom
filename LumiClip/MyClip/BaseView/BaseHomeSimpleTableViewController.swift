//
//  BaseHomeSimpleTableViewController.swift
//  MyClip
//
...
protocol BaseHomeSimpleTableViewDelegate {
    func initHeaderFinish()
}

class BaseHomeSimpleTableViewController<T: NSObject>: BaseViewController, UITableViewDataSource, UITableViewDelegate, VideoTableViewCellDelegate, VideoSmallTableViewCellDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout,
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
         trace = Performance.startTrace(name:"Dexuat")
...
//                print("isconfirmSms \(self.isConfirmSms)")
    ...
    func getHotChannels() {
        userService.getHotChannels() { (result) in
            switch result {
            case .success(let response):
                self.followedChannels = response.data
                self.followViewModel.update(channels: response.data, isAddEmty: true)
    ...
    func initChannelHeader(){
        ...
        mainHeader = UIView(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: 64))
      mainHeader?.backgroundColor = UIColor.setViewColor()
        
        headerView = UICollectionView(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: 64), collectionViewLayout: layout)
        headerView?.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        headerView?.backgroundColor = UIColor.clear
        
        headerView?.dataSource = self
        headerView?.delegate = self
        headerView?.register(UINib(nibName: "ChannelFollowCollectionViewCell", bundle: nil),
                             forCellWithReuseIdentifier: ChannelFollowCollectionViewCell.nibName())
        headerView?.showsHorizontalScrollIndicator = false
        
        mainHeader?.addSubview(headerView!)
        
        let divider = UIView(frame: CGRect(x: 0, y: 63, width: view.frame.width, height: 1))
//        divider.backgroundColor = UIColor.colorFromHexa("DFDFDF")
      divider.backgroundColor  = UIColor.setDarkModeColor(color1: UIColor.colorFromHexa("DFDFDF"), color2: .black)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: rowModel.identifier, for: indexPath) as! ChannelFollowCollectionViewCell
        if(indexPath.row == followViewModel.channelFollows.count - 1){
            cell.bindingWithModel(rowModel, true)
        }else{
            cell.bindingWithModel(rowModel, false)
        }
        return cell
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if(followedChannels.count > indexPath.item){
    ...
    func gotoVideoDetail(_ viewController: BaseHomeSimpleTableViewController,
