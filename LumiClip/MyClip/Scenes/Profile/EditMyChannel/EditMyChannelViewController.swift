...
class EditMyChannelViewController: BaseViewController {
   @IBOutlet weak var coverImage: UIImageView!
   @IBOutlet weak var avatarImage: UIImageView!
   @IBOutlet weak var channelStatusLabel: UILabel!
   ...
   init(model :  EditMyChannelViewModel) {
      ...
      self.channelViewModel = model
       super.init(nibName: "EditMyChannelViewController", bundle: nil)
   }
   ...
   override func viewDidLoad() {
       super.viewDidLoad()
       if(channelViewModel.status == 0){
           self.channelStatusLabel.text =  String.dang_cho_duyet
       }else if(channelViewModel.status == 1){
           self.channelStatusLabel.text = String.da_duyet
       }else{
           self.channelStatusLabel.text = "\(String.tu_choi): \(String.ly_do.lowercased()) " + channelViewModel.reason
       }
       ...
       setupView()
       setupAvatar()
        self.channelStatusLabel.text = ""
       fullNameLabel.text = String.ten_day_du
       motaLabel.text = String.mo_ta
       if channelViewModel.reason == ""  {
           stateLabel.isHidden = true
       }
       stateLabel.text = String.trang_thai
...
extension EditMyChannelViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate, CropViewControllerDelegate {
